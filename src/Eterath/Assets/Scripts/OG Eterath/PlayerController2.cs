...
public class PlayerController2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        newAngle -= (transform.eulerAngles.y) / Mathf.Rad2Deg;
        float xDir = Mathf.Cos(newAngle) * indicatorDistance;
        float zDir = Mathf.Sin(newAngle) * indicatorDistance;
        Debug.Log("XDIR: " + (angle * Mathf.Rad2Deg) + " " + "ZDIR: " + transform.eulerAngles.y + " THIS: " + (newAngle * Mathf.Rad2Deg));
        //xDir += Mathf.Cos(transform.eulerAngles.y);
        //zDir += Mathf.Sin(transform.eulerAngles.y);

        //Debug.Log("XDIR: " + xDir + " " + "ZDIR: " + zDir);
        indicator.transform.position = new Vector3(player.transform.position.x + xDir, player.transform.position.y+2f, player.transform.position.z + zDir);
        indicator.transform.eulerAngles = new Vector3(0, -((Mathf.Rad2Deg * (newAngle)) - 90), 0);
        moveWithCamera.transform.eulerAngles = new Vector3(0,-((Mathf.Rad2Deg*(newAngle))-90), 0);
        ...
        {
            limit = moveWithCamera.transform.eulerAngles.y - 90;
            rotateTo(limit, turnSpeed, playerSkeleton);
            if (playerAnimator.speed < 0)
            {
                playerAnimator.SetFloat("speedWalk", playerAnimator.GetFloat("speedWalk") * (-1f));
            }
        }
        if (a && s)
        {
            limit = moveWithCamera.transform.eulerAngles.y - 315;
            rotateTo(limit, turnSpeed, playerSkeleton);
            if (playerAnimator.speed > 0)
            {
                playerAnimator.SetFloat("speedWalk", playerAnimator.GetFloat("speedWalk") * (-1f));
        ...
        {
            limit = moveWithCamera.transform.eulerAngles.y - 270;
            rotateTo(limit, turnSpeed, playerSkeleton);
            if (playerAnimator.speed < 0)
            {
                playerAnimator.SetFloat("speedWalk", playerAnimator.GetFloat("speedWalk") * (-1f));
        ...
        {
            limit = moveWithCamera.transform.eulerAngles.y - 360;
            rotateTo(limit, turnSpeed, playerSkeleton);
            if (playerAnimator.speed > 0)
            {
                playerAnimator.SetFloat("speedWalk", playerAnimator.GetFloat("speedWalk") * (-1f));
        ...
        if (d && s)
        {
            limit = moveWithCamera.transform.eulerAngles.y - 45;
            rotateTo(limit, turnSpeed, playerSkeleton);
            if (playerAnimator.speed > 0)
            {
                playerAnimator.SetFloat("speedWalk", playerAnimator.GetFloat("speedWalk") * (-1f));
        ...
        if (Input.GetKey(KeyCode.LeftShift))
        {
            ...
            playerSkeleton.transform.eulerAngles = new Vector3(playerSkeleton.transform.eulerAngles.x, playerSkeleton.transform.eulerAngles.y, 0);
        }
        else
        {
        ...
        if (!((Input.GetAxis("Vertical") != 0 || Input.GetAxis("Horizontal") != 0) && !areRunning) && !(Input.GetKey(KeyCode.LeftShift))) 
        {
            playerSkeleton.transform.eulerAngles = new Vector3(playerSkeleton.transform.eulerAngles.x, player.transform.eulerAngles.y, 0);
        }
